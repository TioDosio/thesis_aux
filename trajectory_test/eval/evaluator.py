...
class Evaluator:
    def __init__(self, args):
        ...
        if self.eval_mode in ["loc", "traj_pred"]:
            ...
            if self.load_loc != "":
                self.loc_model.load_state_dict(torch.load(self.load_loc, map_location=torch.device('cpu'), weights_only=True))
        ...
        if self.eval_mode in ["traj_pred"]:
            ...
            if self.load_traj != "":
                self.traj_model.load_state_dict(torch.load(self.load_traj, map_location=torch.device('cpu'), weights_only=True))
    ...
    def evaluate_loc(self):
        ...
        if epoch_traj_count > 0:
            epoch_ade = epoch_ade / epoch_traj_count
            epoch_fde = epoch_fde / epoch_traj_count
            print(f"ADE: {epoch_ade}, FDE: {epoch_fde}")
        else:
            print("Warning: No trajectories found for evaluation. ADE and FDE cannot be computed.")
            print("This usually means the dataset is empty or no valid sequences were generated.")
    ...
    def evaluate_traj_pred(self):
        ...
        with torch.no_grad():
            ...
            if total_samples > 0:
                ...
                print("Warning: No samples found for trajectory prediction evaluation.")
                print("This usually means the dataset is empty or no valid sequences were generated.")
                return 0.0, 0.0