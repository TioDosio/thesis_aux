...
This script subscribes to ROS topics (/tf, /raw_bodies, /image_detections),
...
try:
    ...
    try:
        ...
        HUMAN_AWARENESS_AVAILABLE = True
    except ImportError:
        print("human_awareness_msgs not available. Please build the catkin workspace.")
        HUMAN_AWARENESS_AVAILABLE = False
        PersonTracker = None
    ROS_AVAILABLE = True
except ImportError as e:
    print("ROS packages not available: {}".format(e))
    ROS_AVAILABLE = False

# Try to import our real-time data processor
try:
    from real_time_data_processor import (
        RealTimeDataProcessor, 
        process_tf_message_dict,
        process_image_detections_dict,
        process_raw_bodies_dict
    )
    DATA_PROCESSOR_AVAILABLE = True
except ImportError as e:
    print("Real-time data processor not available: {}".format(e))
...
try:
    ...
    print("Model components not available: {}".format(e))
...
class RealTimeTrajectoryPredictor:
    def __init__(self):
        if not ROS_AVAILABLE:
            print("Cannot initialize without ROS. Please source your ROS environment.")
            sys.exit(1)
            
        if not HUMAN_AWARENESS_AVAILABLE:
            print("human_awareness_msgs not available. Please build the catkin workspace:")
            print("cd ~/catkin_ws && catkin_make && source devel/setup.bash")
            sys.exit(1)
            
        if not DATA_PROCESSOR_AVAILABLE:
            print("Data processor not available. Please check real_time_data_processor.py")
            sys.exit(1)
            
        rospy.init_node('real_time_trajectory_predictor', anonymous=True)
        
        # Configuration - Match model's expected parameters
        self.seq_len = 10   # Must match model config (not 5)
        self.interval = 5   # Reduced from 15  
        self.obs_len = 4    # Must match model config (not 3)
        self.pred_len = 6
        
        # Initialize data processor
        self.data_processor = RealTimeDataProcessor()
        
        # Store the latest odom frame as reference
        self.latest_odom_frame = None
        self.latest_odom_timestamp = None
        
        # Model setup (optional - will continue without model if loading fails)
        if MODEL_AVAILABLE:
            self.setup_model()
        else:
            rospy.logwarn("Model components not available. Running in data collection mode only.")
            self.model = None
        ...
        rospy.loginfo("Waiting for data from topics: /tf, /image_detections (PersonsList)")
        rospy.loginfo("Minimum {} frames needed to start predictions".format(self.obs_len))
        rospy.loginfo("Optimal {} frames ({}*{} interval) = ~{} seconds at 1.25Hz".format(
            self.seq_len * self.interval, self.seq_len, self.interval, 
            (self.seq_len * self.interval) / 1.25))
    ...
    def tf_callback(self, msg):
        """Process transform messages to extract and store the latest odom frame"""
        # print(f"DEBUG: tf_callback triggered - received {len(msg.transforms)} transforms")
        
        odom_transforms = []
        
        # Find all transforms with frame_id='odom' in this message
        for transform in msg.transforms:
            # print(f"DEBUG: Transform - frame_id: {transform.header.frame_id}, child_frame_id: {transform.child_frame_id}")
            if transform.header.frame_id == 'odom':
                odom_transforms.append(transform)
        
        # Use the last odom transform in this message (most recent)
        if odom_transforms:
            # print(f"DEBUG: Found {len(odom_transforms)} odom transforms, using the last one")
            latest_transform = odom_transforms[-1]  # Last one in the message
            ...
            self.latest_odom_timestamp = latest_transform.header.stamp.to_sec()
            ...
            tf_dict = {
                'transforms': [{
                    ...
                    'transform': {
                        'translation': self.latest_odom_frame['translation'],
                        'rotation': self.latest_odom_frame['rotation']
                    }
                }]
            }
            ...
            if ego_data:
                ego_data['timestamp'] = self.latest_odom_timestamp
                ego_data['seq'] = self.latest_odom_frame['seq']
                self.data_processor.add_ego_pose(ego_data)

    def get_current_reference_frame(self):
        """Get the current reference frame (latest odom transform)"""
        return self.latest_odom_frame
    ...
    def image_detections_callback(self, msg):
        ...
        rospy.loginfo("Frame {}: {} persons, Ego poses: {} (min:{}, opt:{}), Active persons: {}".format(
    ...
    def predict_single_trajectory(self, model_input):
        ...
        try:
            with torch.no_grad():
                ...
                print(f"DEBUG: Input tensor shape: {inputs.shape}")
                ...
                if len(inputs.shape) == 4:
                    ...
                    if features == 3 and num_keypoints == 17:
                        ...
                        print(f"DEBUG: Converted from [batch, seq, 3, 17] to [batch, seq, 17, 2]: {inputs.shape}")
                    else:
                    ...
                    if features == 34:  # 17 keypoints * 2 values (x,y)
                        ...
                        print(f"DEBUG: Reshaped from [batch, seq, 34] to [batch, seq, 17, 2]: {inputs.shape}")
                        ...
                        print(f"DEBUG: Converted from [batch, seq, 51] to [batch, seq, 17, 2]: {inputs.shape}")
                    else:
                ...
                print(f"DEBUG: Final input shape for joint2traj: {inputs.shape}")
                ...
                print(f"DEBUG: loc_outputs shape: {loc_outputs.shape}")
                print(f"DEBUG: ego_pose_tensor shape: {ego_pose_tensor.shape}")
                ...
                try:
                    ...
                    print(f"DEBUG: Using localization outputs directly: {traj_estimated_ls.shape}")
                    
                except Exception as e:
                    print(f"DEBUG: Direct approach failed: {e}")
                    ...
                    try:
                        ...
                        print(f"DEBUG: Simplified camera pose failed: {e2}")
                        ...
                        print(f"DEBUG: Manual trajectory creation: {traj_estimated_ls.shape}")
    ...
    def run(self):
        ...
        rospy.loginfo("Topics subscribed: /tf, /image_detections")
        ...
        def diagnostic_timer(event):
            ...
            print(f"📊 DIAGNOSTIC: Ego poses collected: {len(self.data_processor.ego_poses)}")
            ...
            try:
                ...
                print(f"  /tf: {'✓ FOUND' if tf_exists else '✗ MISSING'}")
